...
void LocatarGui2::reload(vector<Locatar> apartamente) {
	model->setLocatars(apartamente);
	table->update(); // e degeaba 
}



void LocatarGui2::connectSignals()
{
	QObject::connect(table->selectionModel(), &QItemSelectionModel::selectionChanged, [&]() {
		...
		edit_del_nr->setText(cel0Val);
		edit_del_nume->setText(cel1Val);
		edit_del_sup->setText(cel2Val);
		edit_del_tip->setText(cel3Val);

		});

	QObject::connect(addLocatar, &QPushButton::clicked, this, &LocatarGui2::guiAdd);

	addLocatar->setStyleSheet("background-color: #fe7ee0");

	QObject::connect(modificaProp, &QPushButton::clicked, this, &LocatarGui2::guiMod);

	modificaProp->setStyleSheet("background-color: #fe7ee0");

	QObject::connect(delAp, &QPushButton::clicked, this, &LocatarGui2::guiDel);
